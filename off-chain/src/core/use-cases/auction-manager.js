...
class AuctionManager {
  ...
  addToken(listId, _token) {
    if (!_token.tokenId || ! _token.minPrice) {
      throw Error('All tokens must has tokenId and minPrice');
    }

    const list = this.#auctionStorage.getList(listId);

    const token = this.#tokenFactory.createToken(_token.listId, _token.minPrice);
  ...
  makeBid(ownerAddr,
    ...
    this.#auctionStorage(list);
  }

  deleteBid(listId, tokenId, bidderAddr) {
    const list = this.#auctionStorage.getList(listId);

    const token = list.getToken(listId);
